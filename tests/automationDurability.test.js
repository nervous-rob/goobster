...
async function createDueHourlyAutomation(name = 'hourly lab check') {
...
describe('hourly execution', () => {
    test('a due hourly automation runs through the chat pipeline and advances to the next hour', async () => {
        const created = await createDueHourlyAutomation();
        ...
        const row = await db.get('SELECT lastRun, nextRun, isEnabled FROM automations WHERE id = @id', { id: created.id });
    ...
    test('nextRun is advanced BEFORE the run executes (claim-before-run)', async () => {
        ...
        handleChatInteraction.mockImplementation(async () => {
            nextRunDuringExecution = (await db.get(
                'SELECT nextRun FROM automations WHERE id = @id', { id: created.id }
            )).nextRun;
        });

        await service.executeAutomation((await service.getDueAutomations())[0]);
...
describe('restart recovery (persistence)', () => {
    test('a fresh service instance over the same database picks up the stored schedule', async () => {
        ...
        const due = await rebooted.getDueAutomations();
        expect(due.map(a => a.id)).toEqual([created.id]);
    ...
    test('paused automations stay paused across a restart and are never due', async () => {
        await createDueHourlyAutomation();
        await automationManagerService.setEnabled({ userId: USER, scope: GUILD, name: 'hourly lab check', enabled: false });

        const rebooted = new AutomationService(makeClient().client);
        expect(await rebooted.getDueAutomations()).toHaveLength(0);
    });
});
...
describe('unparseable schedules', () => {
    test('a row whose cron cannot parse is disabled and the owner notified - never rescanned forever', async () => {
        ...
        const row = await db.get(`SELECT isEnabled, nextRun FROM automations WHERE name = 'broken'`);
...
describe('failure notification (once per streak)', () => {
    const backdate = async (id) =>
        await db.run(`UPDATE automations SET nextRun = datetime('now', '-1 minute') WHERE id = @id`, { id });
...
describe('failure handling (retry semantics)', () => {
    test('a failing run leaves the automation enabled and waits for the next fire - no retry storm', async () => {
        ...
        expect((await db.get('SELECT lastRun FROM automations WHERE id = @id', { id: created.id })).lastRun).toBeTruthy();
    });
});
