...
- **Goobster plays too** (`services/tableGames/botPlayer.js`): a side-effect service — never engine code — that subscribes to a table like any client, watches its own personalized view, and acts through `tableManager.act` (seated with an `isBot` flag only trusted server code can set; the WS layer never forwards it). Decisions per game come from `ADVISORS` (currently hold'em): `buildDecisionContext(view)` serializes game metadata into an ONLY-JSON prompt (and accepts `images` — the extension point for feeding table screenshots to vision models), `legalize()` clamps/repairs whatever the model returns into a legal move, and `fallback()` is a deterministic heuristic used when no AI provider responds; a rejected action retreats to check/fold. The bot banks through the normal economy (`bot-bankroll` top-ups when low), is invited/dismissed via WS `invite-bot`/`dismiss-bot` (inviter must be seated), and auto-leaves when the last human stands. Table talk goes to Activity clients via `tableManager.notify` (`chat` messages), optionally to the channel (`activity.bot.textComments`) and through a live voice session (`activity.bot.voiceComments`, reusing `voiceSessionService` — it never joins voice itself). All bot AI calls carry `usageContext` and rate-limit comments per table.
