...
| `/spotdl` says `spotdl CLI not found` | The error now ends with `Tried: ...` listing every location checked and why it failed — read that first. `exited with code 1: ModuleNotFoundError` means the venv was orphaned by an OS Python upgrade: rerun `./scripts/install-rpi.sh` (or wait for the next auto-update — both rebuild a broken venv automatically), or by hand `python3 -m venv --clear ~/.local/goobster-venv && ~/.local/goobster-venv/bin/pip install spotdl yt-dlp` (plain `pip install` is blocked on Bookworm). The bot checks that venv path automatically |
