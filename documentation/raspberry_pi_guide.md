...
- The **Settings** tab manages everything the settings slash commands do: proactive mode, dynamic responses, reply detection, search approval, thread preference, AI provider/model/reasoning (including Thoughtful Mode), personality directive, bot nickname, memory retention, per-channel memory exclusions, forget-all-memories, and the global ElevenLabs TTS voice.
- The panel has a **built-in on-screen keyboard** (it appears automatically when a text field is focused), so you don't need Squeekboard or any OS keyboard — which wouldn't appear over a fullscreen `--kiosk` window on Wayland anyway.
...
| `/spotdl` or `/play` says `spotdl`/`yt-dlp` `CLI not found` | The error now ends with `Tried: ...` listing every location checked and why it failed — read that first. `not found` on every candidate (including `~/.local/goobster-venv`) means the venv was never created: rerun `./scripts/install-rpi.sh` or `./scripts/ensure-music-cli.sh`. `exited with code 1: ModuleNotFoundError` means the venv was orphaned by an OS Python upgrade: the same commands rebuild it (auto-update does too — it now creates a *missing* venv, not only a broken one), or by hand `python3 -m venv --clear ~/.local/goobster-venv && ~/.local/goobster-venv/bin/pip install spotdl yt-dlp` (plain `pip install` is blocked on Bookworm). The bot checks that venv path automatically |
| Spotify **playlist** downloads fail (`KeyError: 'ownerV2'` / `Playlist not available or not accessible`) but tracks and albums work | spotdl 4.5+'s anonymous Spotify client is broken for playlists by upstream Spotify API changes. Add `"spotify": { "clientId", "clientSecret" }` to `config.json` (free app at the [Spotify Developer Dashboard](https://developer.spotify.com/dashboard)) — albums still use that API; public playlists are expanded from Spotify's embed page and downloaded in batches |
| `/spotdl` or `/play` dies with `Spotify API 403` on `/v1/playlists/{id}/tracks` | Spotify removed that endpoint in February 2026 (it 403s for everyone). Pull a build that falls back to the public embed page. If you still get "will not list that playlist", the list is not actually public — invite links with `?pt=` redirect to login. In Spotify: playlist → **…** → **Make public**, wait a minute, paste the link again |
| `/spotdl` says `Spotify rejected the access token` even with a valid client id/secret | That 401 is on `GET /playlists/{id}/items` after `/api/token` succeeded. Client-credentials cannot list playlists (no logged-in user). Pull a build that skips that call and uses the embed page. Credentials are still used for albums and for spotdl itself |
| Large / shared playlist dies in ~10s with `too many 404 error responses` (often logged as a 429) | That is a **playlist 404**, not a size or rate-limit problem. Spotify cannot open private or invite-only playlists (share links with `?pt=`). Make the playlist public, or paste a public playlist/album/track link. Public lists are expanded from the embed page (first ~100 tracks) and downloaded in batches of 15 (`spotdl.batchSize` / `spotdl.maxPlaylistItems` in `config.json`) |
