...
| Spotify **playlist** downloads fail (`KeyError: 'ownerV2'` / `Playlist not available or not accessible`) but tracks and albums work | spotdl 4.5+'s anonymous Spotify client is broken for playlists by upstream Spotify API changes. Add `"spotify": { "clientId", "clientSecret" }` to `config.json` (free app at the [Spotify Developer Dashboard](https://developer.spotify.com/dashboard)) — Goobster then expands the playlist through the official API and downloads tracks in batches |
| Large / shared playlist dies in ~10s with `too many 404 error responses` (often logged as a 429) | That is a **playlist 404**, not a size or rate-limit problem. Spotify's official API cannot open private or invite-only playlists (share links with `?pt=`). Make the playlist public, or paste a public playlist/album/track link. Public lists are expanded page-by-page and downloaded in batches of 15 (`spotdl.batchSize` / `spotdl.maxPlaylistItems` in `config.json`) |
