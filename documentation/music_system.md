...
**Playlists** cannot be listed through a hobby Spotify app anymore. In February 2026 Spotify removed `GET /playlists/{id}/tracks` (it now 403s) and `GET /playlists/{id}/items` only returns contents for playlists the logged-in user owns. Client-credentials has no user, so Goobster expands **public** playlists from Spotify's embed page instead, then downloads track URLs in batches of 15 (`spotdl.batchSize`). The embed page typically exposes the first ~100 tracks. Invite/share links with `?pt=` are not public — make the playlist public (not just "share") or paste an album/track link. **Albums** still use the official `/v1/albums/{id}/tracks` API (credentials recommended). Optional `spotdl.maxPlaylistItems` caps the expansion (unset = no cap).
