...
**Status: increments A, B, B.1 and C are shipped; E is in progress; D, F and G are pending.** Native login, verified email, the independent runtime and the Inbox exist today - see the [implementation status](#implementation-status) table, which is the authority on what has landed, and the shipped behaviour guides ([identity.md](identity.md), [independent_runtime.md](independent_runtime.md), [portal_navigation.md](portal_navigation.md)). The remaining sections describe the agreed design; a feature described here is not available merely because this document exists.
