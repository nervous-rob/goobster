...
The [guided tutorial specification](guided_tutorials_spec.md) is the authoritative contract. **F1** (framework: state machine, API, Settings, provider shell) is shipped; **F2** authors the demonstration tours. The release gates in this document also apply.
...
| F — Guided onboarding | **F1 shipped** (framework: `tutorialCatalog.js`, `tutorialService.js`, `tutorial_*` tables, `/api/app/tutorials*`, Settings → Tutorials, `TutorialProvider` shell). F2 (authored tour steps) not started. Behaviour: [guided_tutorials_spec.md](guided_tutorials_spec.md). | F1: `tests/tutorialFramework.test.js` on SQLite and Postgres (unknown id rejected; stale generation after reset; skip_step ≠ complete_step; skip one leaves another not_started; unavailable steps recorded not completed; reset one / reset all leave notes and `hiddenToolRooms` alone; auto-start preference only; `/forget-me` / audit / transparency; service does not import AI/transfer writers); `tests/portalRooms.test.js` catalog parity with `rooms.cjs`; `e2e/tutorials.spec.js` (Settings Resume / Replay / Reset one / Reset all, auto-start toggle, unknown id, stale generation, public share never starts a tour — no provider). |
