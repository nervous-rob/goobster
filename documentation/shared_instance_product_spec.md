...
| Gap — Token budgets ([#248](https://github.com/nervous-rob/goobster/issues/248)) | **Shipped in PR #279; both-engine and browser CI passed.** `usageBudgetService` reserves before admission, settles from provider usage and flags uncertain outcomes; bounded background waits, Host → Limits, own Usage budget, account-creation gate, and 90-day configurable retention. Caps remain unset by default. Guide: [work_ledger.md](work_ledger.md#budgets). | `tests/usageBudgets.test.js`: concurrent same-payer holds, forked-worker state, terminal settlement, streaming usage/cost join, cancelled/refused admission, uncertain outcomes, bounded budget waits, erasure, operator audit and `/me`, second-account gate. Included in both-engine CI. |
| G — Private single-user pilot (#265) | Follow one topic weekly, verify evidence, use a brief; measure repeat use, time, quality, cost and failures | [Pilot plan](pilot_plan.md), work ledger, private cycle records | Owner records cycles and an exit decision. A second account is a separate promotion requiring #247 host isolation, #248 daily cap and #255 phase 1. |
...
The private single-user pilot uses [pilot_plan.md](pilot_plan.md): proposed six weekly cycles, owner-selected topic and thresholds, a dated host recovery test before starting, and a recorded exit decision. These are experiment criteria, not completed evidence.

For a later second-account pilot, additional success criteria are: two independent native accounts can finish the core journey without Discord; a shared project works through explicit membership; another account cannot access it; tutorials remain optional and recoverable; one user's long research does not prevent another from ordinary navigation and queued chat; the host can suspend an account and revoke its active sessions/work.
...
| F — Guided onboarding | **#272 adds Back, step feedback, Research v2, and safe Projects basics / Plans / Runs / Inbox / Scheduled v2 previews; Memory, Settings, Usage and operator-only Host v2 now complete the authored batch-1 curriculum. Later batches and human validation remain. F1 + F2 sample tours shipped; #266 provider-free first-task workflow implemented, five observed sessions pending.** Framework: `tutorialCatalog.js`, `tutorialService.js`, `tutorial_*` tables, `/api/app/tutorials*`, Settings → Tutorials, `TutorialProvider` / `TutorialPanel`. F2 authors `home.orientation`, `chat.basics`, `knowledge.basics`, and `projects.apps` with Weekend field notebook samples (`tutorialSamples.js`) and **Keep this example**; the five workflow tours use `workflowTutorials.js` and the four account/Host tours use `accountTutorials.js`; unimplemented catalog entries remain empty. Behaviour: [guided_tutorials_spec.md](guided_tutorials_spec.md). | F1+F2: `tests/tutorialFramework.test.js` on SQLite and Postgres (unknown id rejected; stale generation after reset; skip_step ≠ complete_step; skip one leaves another not_started; unavailable steps recorded not completed; reset one / reset all leave notes and `hiddenToolRooms` alone; auto-start preference only; `/forget-me` / audit / transparency; tour events never write knowledge; Keep copies an allow-listed note once; authored tours v2 with demos); `tests/portalRooms.test.js` catalog parity with `rooms.cjs`; `e2e/tutorials.spec.js` (Settings Resume / Replay / Reset, demos, skip step, Keep this example, Finish — no provider; public share never starts a tour). |
...
| Gap — Pilot diagnostics and cost ([#256](https://github.com/nervous-rob/goobster/issues/256)) | **Shipped.** Three tables keyed by the work reference `(workKind, workId)`: `work_failures` (every listed kind - chat turns, expeditions, jobs, sandbox runs, automations, trigger deliveries, reminders, watches, Inbox echoes - writes kind, work id, phase, code, a short reason and actor; never a prompt or body), `resource_events` (search calls, sandbox seconds, retries, under `utils/workContext.js`), and `operator_audit` (every Host-room action, pause / restore / resume, one-year retention). `usage_reservations` supplies the cost join through #248’s reservation writer. `services/costReportService.costPerResult` joins settled tokens with resource events per accepted result. The person sees own rows in Usage → *What went wrong* and on the Inbox item that reports the failure; the operator sees any account's usage + resources + failures (Host → Accounts → *Support*) and the audit (Host → *Operator audit*). Retention 30 / 90 / 365 days (`ledgerRetentionService`, coreRuntime); erasure nulls the actor and keeps the row. Guide: [work_ledger.md](work_ledger.md). | `tests/workLedger.test.js` on SQLite and Postgres: every kind writes a row with no marker text; a failed expedition, scheduled task, sandbox run, job and Discord echo each through the real path (expedition and automation with a linked Inbox item); per-provider search calls under the expedition; per-person totals; the seeded cost-per-result join (1500 tokens, 3 searches, 12.5 sandbox seconds, 1 retry over 1 accepted result); own rows only; the operator's per-account view and roster counts; 30 / 90 / 365-day sweeps and runtime registration; erasure nulls and keeps (reservations paid by the person deleted); nine operator actions in the Host room each write one audit row with no token or link stored; paging, filters, secret keys stripped. |
| Gap — Token budgets ([#248](https://github.com/nervous-rob/goobster/issues/248)) | **Shipped in PR #279; both-engine and browser CI passed.** `usageBudgetService` reserves before admission, settles from provider usage and flags uncertain outcomes; bounded background waits, Host → Limits, own Usage budget, account-creation gate, and 90-day configurable retention. Caps remain unset by default. Guide: [work_ledger.md](work_ledger.md#budgets). | `tests/usageBudgets.test.js`: concurrent same-payer holds, forked-worker state, terminal settlement, streaming usage/cost join, cancelled/refused admission, uncertain outcomes, bounded budget waits, erasure, operator audit and `/me`, second-account gate. Included in both-engine CI. |
| Gap — Research evaluation ([#267](https://github.com/nervous-rob/goobster/issues/267)) | **Harness merged (PR #281); owner baseline pending.** [research_evaluation.md](research_evaluation.md): 30 fictional fixed-evidence questions, opt-in provider calls, owner claim marks/edits and isolated work-ledger costs. | `tests/researchEvaluation.test.js` checks machinery on both engines. A live generation pass is not a factual-quality pass; the owner must record the baseline. Live source discovery is not covered. |
| Stage 3 — Research brief ([#254](https://github.com/nervous-rob/goobster/issues/254)) | **Implemented on a branch; PR open, not merged.** [research_brief.md](research_brief.md): a private brief written once from a finished Expedition's stored sources and claims (`expedition_briefs`, generated text + SHA-256, hash-verified on read), inline `[n]` citations to stored claims, model-stated limitations plus evidence-derived notes (uncited, weak evidence, disagreement, missing coverage, dates), a separate edit overlay marked wording/factual with `EDIT_CONFLICT` guarding, #267's marks and gates with the quality status computed on read (`unreviewed` is never a pass), explicit and reversible acceptance and use, Markdown export that marks edited passages and states the status, owner-only access (404 for strangers, project owners and members), generation inside the expedition's work reference with the project owner as payer for project expeditions, failed attempts kept with a `work_failures` `brief` row, erasure/audit/transparency coverage, and `GET /api/app/spitball/briefs/measure` (tokens and resources as separate units, `perAccepted: null` with nothing accepted). Share links, templates and scheduled briefs deferred. | `tests/expeditionBrief.test.js` on SQLite and Postgres and `e2e/researchBrief.spec.js` prove the machinery. They are not the owner-judged baseline (#267) and not a pilot result (#265); acceptance and use are recorded by the owner in pilot cycles. |
| G — Private single-user pilot ([#265](https://github.com/nervous-rob/goobster/issues/265)) | **Plan written; execution not started.** [pilot_plan.md](pilot_plan.md) proposes six weekly cycles, the recurring task, quality/time/cost/failure measures and three exit choices. Topic, dates and thresholds await the owner. | No results claimed. Record the actual-host recovery test before cycle 1; capture weekly evidence and the exit decision before closing #265. Second-account gates remain #247, #248 and #255. |
