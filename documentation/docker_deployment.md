...
- `/app` and `/api/app/*` → api:3100. `/app` is the React SPA after
  `npm run build:web`. `/app/next` redirects to `/app`. SSE
  (`/api/app/events` and chat/parlor turns) uses `proxy_buffering off`
