...
- **Map tab** (`GET /api/app/memory/constellation`) renders the **real** user-scoped graph: `kg_nodes` + `kg_edges` + tags, up to the storage cap (2500 personal). A `person` anchor node represents the user. Search, type, tag, and source filters hide nodes client-side so a dense graph stays navigable; a hit list pans to the chosen note. **Group by tag** (on by default, remembered in `localStorage`) overlays tag hubs. A small map keeps every tag as a diamond; a dense map keeps root clusters and smaller satellite groups (`utils/graphClusters.js`), interconnects them with parent/child and shared-tag overlap edges, springs each note to its primary group and weakly to the others it shares, and still uses cluster hulls plus a soft third axis. The overlay is view-time only — never written to `kg_nodes` or `kg_edges`.
