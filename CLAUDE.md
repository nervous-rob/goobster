...
### The work ledger (`documentation/work_ledger.md`)
`work_failures`, `resource_events` and `operator_audit` (plus #248's `usage_reservations`) key on the work reference `(workKind, workId)` carried by `utils/workContext.js` (`AsyncLocalStorage`). In a failure path call `workFailureService.note({ phase, code, reason })` (or `notify()` to also file an Inbox item); for a non-token cost call `resourceEventService.record({ kind, quantity, provider })`; both default kind/id/actor from the context and never throw. **Never put a prompt, reply, body, output, token, link or address in a ledger row.** Every mutating operator route writes one `operator_audit` row after it succeeds. Erasure nulls `actor` and keeps the row; `ledgerRetentionService` prunes at 30/90/365 days.
