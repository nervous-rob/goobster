...
### Changed
- **Mission execution hardening.** Approval is now a real boundary: the portal mints a short-lived confirmation receipt bound to the current `planRevision`, `approve()` consumes it, and any later plan mutation (edit or add/skip a step) increments the revision, clears the approval, and returns the mission to `DRAFT`. The observatory tool can no longer approve or start a mission. Step execution is `READY → STARTING → RUNNING` with an attempt key — required job/watch parameters are validated before the claim, concurrent `startStep` calls lose the atomic UPDATE, and stranded `STARTING` rows are reconciled. Cancel and skip cancel the linked job, expedition, or watch before downstream steps become ready. Attention deadline notices no longer require a recent `updatedAt`, so an untouched two-week mission still surfaces when its due date enters the 72-hour horizon. Automatic evidence is `supported` / `contested` / `unassessed` (deduped; claims must be on this project unless explicitly imported); `met` / `unmet` / `mixed` stay reserved for the human review. Completing a mission writes at most one `project_decisions` row. Jest: `projectMissionService` / `projectMissionApi`.
