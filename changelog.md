...
### Changed
- **Mission execution hardening.** Approval is now a real boundary: the portal mints a short-lived confirmation receipt bound to the current `planRevision`, `approve()` consumes it, and any later plan mutation (edit or add/skip a step) increments the revision atomically (`planRevision = planRevision + 1` with a status predicate), clears the approval, and returns the mission to `DRAFT`. A zero-row bump throws, so `addStep` cannot race `start()`. The observatory tool can no longer approve, start, or complete a mission — completion also consumes a human `complete` receipt. Step execution is `READY → STARTING → RUNNING` with an attempt key written onto the child job / expedition / watch; expeditions are created as `DRAFT`, linked, then dispatched. Concurrent `startStep` calls lose the atomic UPDATE. `reconcileStartingSteps` runs on bot/api startup and on the personal heartbeat; an unlinked stale claim fails and blocks the mission. A `FAILED` step is resolved by retry (`FAILED → READY`) or skip (`FAILED → SKIPPED`); `resume()` refuses while a failed step remains. Cancel and skip cancel the linked child as its launching owner. Attention deadline notices no longer require a recent `updatedAt`. Automatic evidence is `supported` / `contested` / `unassessed`; existing installs normalize NULL `criterionId` and deduplicate evidence/decisions before those unique indexes are created. Completing a mission writes at most one `project_decisions` row. Jest: `projectMissionService` / `projectMissionApi` / `dbSchemaUpgrade`.
...
### Removed
- **The legacy ES-module web client is gone — the React portal is the only web client.** `packages/core/web/app/` (the pre-port SPA shell, room panes, renderer modules, share viewer, PWA assets) is deleted along with the `webapp.nextClient` rollback knob: `/app` always serves `apps/web/dist`, and a missing build answers `503 WEB_CLIENT_UNBUILT` instead of silently falling back (run `npm run build:web`). The last live dependency on the leftover tree — the read-only conversation share viewer — is ported to a React route (`apps/web/src/rooms/SharePage.tsx`) that renders inside the full app shell (sidebar + room navigation) but **outside** the login gate: the shell tolerates a null session (feature-gated rooms hide, the footer offers Sign in), anonymous viewers read the share without logging in, and room links land them on the login screen. The unguessable token stays the only capability, resolved via the same public `GET /api/app/share/:token`. `/app/next` bookmarks still 302 onto `/app`, Observatory share snapshots remain server-generated HTML, and the React client's own icons/manifest/service-worker/`style.css` already carried the PWA. Jest spec `webNextClient` becomes `webClientServing` (SPA serving, share-as-SPA-route, unbuilt 503, legacy-client-gone guard).
