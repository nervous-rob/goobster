...
### Added
- **Recurring work in chat now becomes a durable automation, never a chained follow-up**: a new `manageAutomation` AI tool (create/list/pause/resume/update/cancel) lets the model turn "check the lab feed every hour" into a row in the `automations` table — the same restart-safe rows `/automation` and the portal Tasks pane manage, executed by `automationService`'s minute poll, with `nextRun` advanced after every fire (success or error) so an occurrence never runs twice. The shared management layer (`services/automationManagementService.js`) owns natural-language→cron conversion (moved out of the `/automation` command), validation (15-minute cadence floor, per-scope cap of 10, duplicate-name refusal), and delivery targeting (current server channel, or the user's Discord DM from DM/web chats). `scheduleFollowUp` is now **strictly one-time by contract**: both the tool and `followupService.schedule()` refuse recurring cadences ("every hour", "daily") and route the model to `manageAutomation` instead. Offered by voice too (transcript-channel sessions). New Jest specs: `automationManagementService`, `automationDurability` (hourly execution, restart recovery, no duplicate runs, failed-run idempotency), `toolsRegistryScheduling` (routing between the two tools) — 49 new tests, 1481 total
...
### Fixed
- `documentation/commands.md` no longer claims automations only trigger while the creating user is online — they are unattended and durable, and have been since the presence gate was removed
