...
module.exports = {
	...
	async execute(interaction) {
		try {
			...
			const user = await db.get('SELECT id FROM users WHERE discordId = @discordId', { discordId: interaction.user.id });
			...
			await db.transaction(async () => {
				await db.run('UPDATE users SET activeConversationId = NULL WHERE id = @userId', { userId: user.id });
				await db.run(
					'DELETE FROM messages WHERE conversationId IN (SELECT id FROM conversations WHERE userId = @userId)',
					{ userId: user.id }
				);
				await db.run('DELETE FROM conversations WHERE userId = @userId', { userId: user.id });
				await db.run('DELETE FROM prompts WHERE userId = @userId', { userId: user.id });
			});
