CREATE TABLE [dbo].[model_responses] (
    ...
    CONSTRAINT [FK_model_responses_message] FOREIGN KEY ([message_id]) REFERENCES [dbo].[messages] ([id]),
    CONSTRAINT [FK_model_responses_model_config] FOREIGN KEY ([model_config_id]) REFERENCES [dbo].[model_configs] ([id]),
    CONSTRAINT [FK_model_responses_user] FOREIGN KEY ([user_id]) REFERENCES [dbo].[UserPreferences] ([userId])
);
GO
...
CREATE NONCLUSTERED INDEX [IX_model_responses_user]
    ON [dbo].[model_responses]([user_id] ASC, [created_at] ASC);
GO
