...
- **Web app**: a Discord-authenticated browser interface served by the bot itself — full streaming chat through the same pipeline (shared DM memory, Markdown with highlighted code, no message-length limits) with the modern chat-app suite (multiple conversations with auto-titles, stop/regenerate/edit & resend, image attachments, copy/export, light & dark themes, Thoughtful Mode), plus a memory dashboard with per-entry delete controls and an interactive knowledge-graph view, and **the Parlor** — a multi-persona AI workspace where each persona keeps a private tag-first knowledge base (seedable notes, semantic search, graph view), grounds every reply in its own notes with traceable context, and files away what it learns from discussions (opt-in; see `documentation/webapp_setup.md`)
