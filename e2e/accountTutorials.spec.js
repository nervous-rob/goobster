...
const tours = require('../packages/core/config/accountTutorials');
const modes = [
...
for (const [ti, tour] of tours.entries()) for (const [mi, mode] of modes.entries()) {
    test(`${tour.id}: ${mode.name}, simulation, Back, feedback, resume and finish`, async ({ page }) => {
        ...
        await login(page, { userId: `99000000000000${ti}9${mi}`, name: 'Account learner' });
        const unexpected = [];
        page.on('request', r => {
...
test('ordinary accounts cannot see or start the Host tour', async ({ page }) => {
    await login(page, { userId: '990000000000009999', name: 'Member learner' });
    await page.goto('/app/settings/tutorials');
    await expect(page.locator('.tutorial-row[data-tutorial-id="admin.instance"]')).toHaveCount(0);
    const response = await page.request.post('/api/app/tutorials/admin.instance/events', {
        data: { action: 'start', eventId: 'host-denied', generation: 1, expectedRevision: 0 }
    });
