...
class HeartbeatService {
     ...
     */
    async tick() {
        const outcome = await db.withSingletonLock('heartbeat', async () => {
        ...
        if (!outcome.acquired) {
            console.warn('[Heartbeat] Tick skipped: another process holds the singleton lock');
            return { skipped: true };
        }
...
Optionally include "mood": "<2-5 word mood reflecting the server vibe right now>".`;
    ...
    async deliverDueFollowups() {
        // Personal follow-ups land in the inbox (then echo to Discord); guild
        // follow-ups post to their channel through this client. The pass and
        // its singleton lock live in followupDeliveryService so the core
        // runtime can run the same pass in a process without a bot.
        const followupDeliveryService = require('./followupDeliveryService');
        return await followupDeliveryService.deliverDue({ client: this.client });
    }
