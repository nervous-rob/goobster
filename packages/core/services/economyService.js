...
class EconomyService {
     ...
     */
    async setCurrencyName(guildId, name) {
     ...
     */
    async setAmounts({ guildId, startingBalance = null, dailyAmount = null }) {
    ...
    async getWallet(guildId, userId) {
        ...
        const { startingBalance } = await this.getSettings(guildId);
    ...
    async getBalance(guildId, userId) {
        return (await this.getWallet(guildId, userId)).balance;
    }
     ...
     */
    async adjust({ guildId, userId, amount, type, detail = null }) {
        if (!Number.isInteger(amount)) {
            throw new EconomyError('BAD_AMOUNT', 'Amount must be a whole number.');
        }
        await this.getWallet(guildId, userId);
        ...
        return await db.transaction(async () => {
            ...
            if (newBalance < 0) {
                const { currencyName } = await this.getSettings(guildId);
    ...
    async transfer({ guildId, fromUserId, toUserId, amount }) {
        ...
        await this.getWallet(guildId, fromUserId);
        await this.getWallet(guildId, toUserId);

        return await db.transaction(async () => {
            const fromBalance = await this.adjust({
                guildId, userId: fromUserId, amount: -amount,
                type: 'transfer-out', detail: JSON.stringify({ to: toUserId })
            });
            const toBalance = await this.adjust({
    ...
    async claimDaily(guildId, userId) {
        const { dailyAmount } = await this.getSettings(guildId);
        if (dailyAmount <= 0) {
        ...
        return await db.transaction(async () => {
            ...
            const balance = await this.adjust({ guildId, userId, amount: dailyAmount, type: 'daily', detail: null });
            await db.run(
